            ...
            var setCookieHeaders = Object.keys(cookies).map(function(name) {
                ...
                if(config.domainPrefixing) {
                    return libCookie.serialize(name, value, {
                        domain: '.' + nextUri.host + (nextUri.port ? '.port-' + nextUri.port + '-port.' : '.') + config.domain,
                        path: '/'
                    });
                } else {
                    return libCookie.serialize(name, value, {
    ...
    function rewriteCookiesAndLinks(data) {
        ...
        if (cookies.length) {
            ...
            data.headers['set-cookie'] = cookies.map(function(cookie) {
                ...
                if(config.domainPrefixing) {
                    // make sure that the cookie.domain can be set for the targetUri
                    debug(cookie.domain);
                    if(cookie.domain && ('.'+targetUri.host).indexOf(cookie.domain) - 1 === targetUri.host.length - cookie.domain.length) {
                        debug("set for cookie.domain");
                        cookie.domain = (cookie.domain.indexOf('.') === 0 ? '' :'.') + cookie.domain + '.' + config.domain;
                    } else {
                        if(TLD.registered(targetUri.hostname) == TLD.registered(uri.hostname)) {
                            cookie.domain = '.' + TLD.registered(targetUri.hostname) + '.' + config.domain;
                        } else {
                            cookie.domain = '.' + targetUri.host + '.' + config.domain;
                        }
                    }
                } else {
        ...
        if (data.redirectUrl) {
            ...
            if (((diffProto && !config.domainPrefixing) || diffHost) && TLD.registered(nextUri.hostname) == TLD.registered(uri.hostname)) {
            //if ((diffProto || diffHost) && TLD.registered(nextUri.hostname) == TLD.registered(uri.hostname)) {
                ...
                var oldSetCookieHeaders = _.mapValues(oldCookies, function(value, name) {
                    if(config.domainPrefixing) {
                        //Karl TODO: consider changing this to the parseCookies function from ldbib
                        return libCookie.serialize(name, value, {
                            domain: '.' + nextUri.host + (nextUri.port ? '.port-' + nextUri.port + '-port.' : '.') + config.domain,
                            path: '/'
                        });
