...
Unblocker.headerPrivacy = require('./headerPrivacy.js');
...
Unblocker.domainPrefixer = require('./domain-prefixer.js');
Unblocker.jsDomainPrefixer = require('./js-domain-prefixer.js');
Unblocker.jsonDomainPrefixer = require('./json-domain-prefixer.js');
...
Unblocker.getRealUrlPrefixed = require('./get-real-url-prefixed.js');

var defaultConfig = {
    prefix: null,
    domainPrefixing: true,
    domain: 'bzdomainproxee-test.herokuapp.com',
...
function Unblocker(config) {
    ...
    if (config.standardMiddleware !== false) {
        ...
        var headerPrivacy = Unblocker.headerPrivacy(config);
        ...
        var domainPrefixer, urlPrefixer;
        if (config.domainPrefixing) {
            domainPrefixer = Unblocker.domainPrefixer(config);
        } else {
            urlPrefixer = Unblocker.urlPrefixer(config);
        }
        var jsDomainPrefixer = Unblocker.jsDomainPrefixer(config);
        var jsonDomainPrefixer = Unblocker.jsonDomainPrefixer(config);
        ...
        config.responseMiddleware = [
            ...
            charsets,
            (config.domainPrefixing ? domainPrefixer : urlPrefixer),
            jsDomainPrefixer,
            jsonDomainPrefixer,
    ...
    var getRealUrlPrefixed = Unblocker.getRealUrlPrefixed(config);

    function handleRequest(clientRequest, clientResponse, next) {
        var uri, data;

        var ipToConnectWith = null;

        if(clientRequest.ipToConnectWith) {
            ipToConnectWith = clientRequest.ipToConnectWith;
        }
        ...
        // only requests that start with h-t-t-p get proxied - the rest get
        // redirected to either a url that matches this or the home page
        if (config.domainPrefixing) { // h-t-t-p-s.www.google.se.unblocker
            if(clientRequest.headers.host.indexOf('h-t-t-p') !== 0) {
                if(clientResponse.writeHead) { // http
                    clientResponse.writeHead(301, {'Location': 'http://h-t-t-p.' + clientRequest.headers.host + clientRequest.url});
                } else if(clientResponse.set) { // express
                    clientResponse.status(301).set({'Location': 'http://h-t-t-p.' + clientRequest.headers.host + clientRequest.url});
                }
                return clientResponse.end();
            }
            uri = url.parse(getRealUrlPrefixed(clientRequest.headers.host, clientRequest.url));
            console.log("uri to visit", uri);
            data = {
                url: url.format(uri),
    ...
    function getRealUrlPrefixed(domain, path) {
        var port = '';
        var re_port = /\.-p([0-9]{1,5})-\./i;
        if (re_port.test(domain)) {
            port = ':' + domain.match(re_port)[1];
            domain = domain.replace(re_port, '.');
        }
        if (domain.indexOf('h-t-t-p-s') === 0) {
            console.log('https://' + domain.substring(10, domain.lastIndexOf(config.domain) - 1).replace('_', '.') + port + path);
        } else {
            console.log('http://'  + domain.substring(8,  domain.lastIndexOf(config.domain) - 1).replace('_', '.') + port + path);
        }
